  ...
  def branchSources = it.sources
    branchSources.each 
    {
        ...
        if(mySrc instanceof com.cloudbees.jenkins.plugins.bitbucket.BitbucketSCMSource) 
        {
            ...
            //def gitJenkinsFileUrl = mySrc['serverUrl'] + "/scm/" + mySrc['repoOwner'] + "/" + mySrc['repository'] + "/browse/jenkinsfile"

            def sout = new StringBuilder(), serr = new StringBuilder()
            //def gitJenkinsFileUrl = mySrc['serverUrl'] + "/scm/" + mySrc['repoOwner'] + "/" + mySrc['repository'] + "/raw/jenkinsfile?at=refs%2Fheads%2Fdevelop"
            def gitJenkinsFileUrl = mySrc['serverUrl'] + "/projects/" + mySrc['repoOwner'] + "/repos/" + mySrc['repository'] + "/raw/jenkinsfile?at=refs%2Fheads%2Fdevelop"
            println "${gitJenkinsFileUrl}"

            def proc = ["curl", gitJenkinsFileUrl].execute()
            //def proc = 'curl https://globalrepository.mclocal.int/stash/projects/QE/repos/cx-java-sample/raw/jenkinsfile?at=refs%2Fheads%2Fdevelop'.execute()
            ...
            //def myStr = ["git", "browse", gitJenkinsFileUrl].execute()
