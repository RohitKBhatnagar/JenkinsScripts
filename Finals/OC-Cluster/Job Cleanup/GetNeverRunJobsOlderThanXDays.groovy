...
/********************************************************************
      GetNeverRunJobsOlderThanXDays
      Compare two lists of never run jobs collected X days apart and determine which jobs are common between the files (older than X days), 
and which are unique (new in the last 10 days).  Save the jobs older than X days in a file and place in artifactory for later disable/delete.
 ********************************************************************/
...
String workspacePath = "/tmp/GetNeverRunJobsOlderThan"
...
int daysOlderThan = 15

println("Find Neverrun jobs older than ${daysOlderThanParam} days old")
...
xDaysAgo = jjcGetDateXDaysAgo(daysOlderThan)
todaysDate = jjcGetDate()
f1 = jjcGetController() + ".data.neverrun." + todaysDate
f2 = jjcGetController() + ".data.neverrun." + xDaysAgo
ap1 = artifactory + artPath + "/" + f1
ap2 = artifactory + artPath + "/" + f2

List<String> result
println("Downloading " + ap1 + " from artifactory")
result = jjcDownloadFromArtifactory(username, password, ap1, "")
if (result[0] != null) {
    println("Result code: " + result[0])
    if (result[0] == "22") {
        println("File not found in artifactory - Terminating job.")
        return
    }
}

println("Downloading " + ap2 + " from artifactory")
result = jjcDownloadFromArtifactory(username, password, ap2, "")
if (result[0] != null) {
    println("Result code: " + result[0])
    if (result[0] == "22") {
        println("File not found in artifactory - Terminating job.")
...
def java.io.File results

println("=================================================================================")
println("Compare " + f1 + " to " + f2)
results = jjcFileCompare(f1, f2)
jjcCleanup(f1)
jjcCleanup(f2)
if (results == null) { //something went wrong
    println("File compare failed - Terminating")
    return
}

ap3 = artifactory + artPath + "/" + results.getName()
List<String> ret = jjcCurlToArtifactory(username, password, ap3, results.getAbsolutePath())
println("Created: ${ap3}")
jjcCleanup(results.getAbsolutePath())
println("=================================================================================")
