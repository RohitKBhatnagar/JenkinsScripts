...
/******************************************************************************
 * GetWorkflowJobsOlderThan (one year or 180 days)
 Get a list of jobs older than a year, disable those jobs, and put the jobs names in a file for later deletion.  Store that file in artifactory.
 ******************************************************************************/
...
String workspacePath = "/tmp/GetJobsOlderThan"
...
if (timePeriod == "OneYear") {
    ...
    if (timePeriod == "180Days") {
        daysDelta = -181
        typeOfFile = '180'
    } else {
        println("There are only two options.  Something is seriously wrong.  Aborting.")
        return
    }
}

Calendar numberOfDays = Calendar.getInstance()
numberOfDays.add(Calendar.DATE, daysDelta)

println("GetJobsOlderThan ${timePeriod} - ActionToTake: ${ActionToTake}")
action = ActionToTake

try {
    outfile = jjcCreateJobCleanupFile(jjcGetController(), "/tmp", typeOfFile, "pending_delete")
}
catch (Exception e) {
        ...
        try {
            if (!jjcIgnoreJob(ignoreList, item.fullName)) {
                if (item.getLastSuccessfulBuild() != null) {
                    ...
                    if (build_time.compareTo(numberOfDays) < 1) {
                        line = ""
                        itemCount++
                        line = item.fullName + "\n"
                        w << line
                        //disable the job
                        if (action == "Disable") {
                            jjcDisableJob(item.fullName)
                        } else {
                            if (action == "Delete") {
                                jjcDeleteJob(item.fullName)
                            }
                        }
                    }
                }
...
println("Jobs That Have Not Run In ${timePeriod}: " + sprintf('%-10s', itemCount))
