...
def folderName="SCSS"; //"MarketingBusinessSolutionsDevOps"; // change value `folder-a` for the full name of the folder you want to disable all jobs in
...
try
{
    ...
    topItems.eachWithIndex { topI, topICount ->
      try
      {    
        iItemCount++;
        if(topI instanceof Folder)
        {
          ...
          println "${iItemCount} | ${topI.getName()} | ${topI.getFullName()} | ${topI.getUrl()} | Class-Name : ${topI.getClass()}"
        }
