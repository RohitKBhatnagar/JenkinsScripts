...
println("Current time is - ${c.time}");
println("Hour - " + c.get(Calendar.HOUR_OF_DAY) + ": Minutes - " + c.get(Calendar.MINUTE));
c.add(Calendar.DATE, -30)
println "Thirty days before - ${c}"; 
c.add(Calendar.MINUTE, -c.get(Calendar.MINUTE))
println "Rounded to top of the HR - ${c}"; 
*****/
def Date before60Minutes;
def Date topHrBefore60Minutes;
Calendar cal = Calendar.getInstance();
use( groovy.time.TimeCategory ) 
{
  before60Minutes = new Date() - 60.minutes
}
cal.setTime(before60Minutes);
cal.add(Calendar.MINUTE, -cal.get(Calendar.MINUTE));
topHrBefore60Minutes = cal.time;
println "Time as date - ${topHrBefore60Minutes}";
...
try
{
    def myItems = allItems.findAll
    {
        try
        {
            ...
            new Date(build.startTimeInMillis).after(topHrBefore60Minutes)
        }
        catch(Exception exp)
        {
            //Do Nothing
    ...
    //println "Build count after - ${before60Minutes} are - ${iCount - 1}"; //topHrBefore60Minutes
    println "Build count after - ${topHrBefore60Minutes} are - ${iCount - 1}"; 
