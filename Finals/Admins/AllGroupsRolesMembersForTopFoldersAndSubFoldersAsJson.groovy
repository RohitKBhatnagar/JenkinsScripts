	...
	containers.eachWithIndex 
    {
		...
		for (c in itC) 
        {
            if(c.value.groups.size() > 0)
		    {
                        ...
                        for (a in g.membership) 
                        {
                            ...
                            if(a instanceof nectar.plugins.rbac.assignees.ExternalGroupAssignee) 
                            {
                                ...
                                if(a.hasProperty('members'))
                                {
                                    ...
                                    //Array List print out
                                    //groupMbrs = [Folder:c.value.containerTarget.fullName, Group:g.name, Role:r + (g.doesPropagateToChildren(r) ? " (and children)" : " (pinned)"), Members:a.id, Total:a.members.size()]
                                    //println groupMbrs;
                                ...
                                {
                                    //Array List print out
                                    //groupMbrs = [Folder:c.value.containerTarget.fullName, Group:g.name, Role:r + (g.doesPropagateToChildren(r) ? " (and children)" : " (pinned)"), Members:a.id]
                                    //println groupMbrs;
                            ...
                            lstMembers.add(ADGroup:a.id, Members:lstADMembers, Count:total);
                        }
                ...
                def lstFolders = [Folder:c.value.containerTarget.fullName, Groups:lstGroups]
                //println "+++++++++"
                //println lstFolders;
                println "+++++++++"
