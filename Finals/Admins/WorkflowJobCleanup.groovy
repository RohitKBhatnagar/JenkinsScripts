...
String workspacePath = "/tmp/WorkflowJobCleaner"
...
println("WorkflowJobCleaner")
println("ActionToTake: ${actionToTakeParam}")
println("Type of File: ${typeOfFileParam}")
println("Status of File: ${statusOfFileParam}")
println("Date of File: ${dateOfFileParam}")
println("Milliseconds between deletes: ${millisecondsBetweenDeletesParam}")
int sleepTime = 2000
if (millisecondsBetweenDeletesParam.isInteger()) {
    sleepTime = millisecondsBetweenDeletesParam.toInteger()
} else {
    println("millisecondsBetweenDeletes must be a postive integer value")
    println("Exiting Job")
    return
}

fileName = jjcGetController() + "." + typeOfFileParam + "." + statusOfFileParam + "." + dateOfFileParam
ap = artifactory + artPath + "/" + fileName

List<String> result
println("Downloading " + ap + " from artifactory")
result = jjcDownloadFromArtifactory(username, password, ap, "")
if (result[0] != null) {
...
fileHandle = new java.io.File(fileName)
...
fileHandle.eachLine { line ->
    ...
    if (!jjcIgnoreJob(ignoreList, line)) {
        if (actionToTakeParam == "Disable") {
            jjcDisableJob(line)
        } else {
            if (actionToTakeParam == "TrickleDelete") {
                jjcTrickleDelete(line, sleepTime)
            } else {
                if (actionToTakeParam == "Delete") {
                    jjcDeleteJob(line)
                } else {
                    if (actionToTakeParam == "PrintFile") {
                        NOOP()
                        println(line)
                    }
                }
...
def List<String> jjcDownloadFromArtifactory(String username, String password, String artifactoryPath, String destinationPath) {
   // cmd = "curl -sSf -u\'" + username + ":" + "\'" + password + "\'" + " -O \"" + artifactoryPath + "\""
     cmd = "curl -sSf -udep-cicd:P1on9e@E -O \"" + artifactoryPath + "\""
    response = jjsExec(cmd)
    if (response[response.length - 1] == "404") {
